...
function RoomList() {
    ...
    const enterChatRoom = (roomname) => {
        ...
        chat.message = `${nickname} entró a la sala`;
        chat.type = 'join';
        ...
        firebase.database().ref('roomusers/').orderByChild('roomname').equalTo(roomname).on('value', (resp) => {
            ...
            if (user !== undefined) {
              ...
              userRef.update({status: 'En Linea'});
            } else {
              const newroomuser = { roomname: '', nickname: '', status: '' };
              newroomuser.roomname = roomname;
              newroomuser.nickname = nickname;
              newroomuser.status = 'En Linea';
    ...
    return (
        <div>
            ...
            <Jumbotron>
                <h3>{nickname} <Button onClick={() => { logout() }}>Salir</Button></h3>
                <h2>Chats</h2>
                <div>
                    <Link to="/addroom">Añadir Chat</Link>
                </div>
                <ListGroup>
                    {room.map((item, idx) => (
